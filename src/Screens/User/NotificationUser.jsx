...
const Notification = () => {
  ...
  const getAllNotification = async () => {
    const data = await fetchApiData(
      "https://flyweisgroup.com/api/api/v1/admin/notifications"
    );
    setAllNotification(data?.data);
  };

  const handleNotification = async (data) => {
    if (data?.appointmentId?.appointmentType?.toLowerCase() === "chat")
      navigate(`/chat/${data?.appointmentId?.lawyer}`);
    else if (
      data?.appointmentId?.appointmentType?.toLowerCase() === "video-call"
    )
    ...
    await updateApiData(
      `https://flyweisgroup.com/api/api/v1/customer/appointmentJoin/${data?.appointmentId?._id}`
    );
  };
